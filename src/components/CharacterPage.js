...
function CharacterPage() {
   ...
   useEffect(() => {
    fetch('http://localhost:3001/characters?_sort=name&_order=desc')
        .then((res) => res.json())
   ...
   const onAddCharacter = (newCharacter) => {
    ...
    const charactersToDisplay = characters.filter(character => character.name.toLowerCase().includes(searchQuery.toLowerCase()))
